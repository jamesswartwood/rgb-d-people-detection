...
template <typename T> static std::string color(T const &value) {
  return "\033[" + std::to_string(static_cast<unsigned int>(value)) + 'm';
}

inline std::string color24_fg(unsigned int red, unsigned int green,
                              unsigned int blue) {
  return "\033[38;2;" + std::to_string(red) + ';' + std::to_string(green) +
         ';' + std::to_string(blue) + 'm';
}

inline std::string color24_bg(unsigned int red, unsigned int green,
                              unsigned int blue) {
  return "\033[48;2;" + std::to_string(red) + ';' + std::to_string(green) +
         ';' + std::to_string(blue) + 'm';
}

inline void write(const std::string &s) { std::cout << s << std::flush; }
...
inline std::string clear_screen_buffer() { return "\033[3J"; }
...
public:
  Window_24bit(size_t w, size_t h)
      : w{w}, h{h}, cursor_x{1}, cursor_y{1}, chars(w * h, ' '),
        m_fg(w * h, {0, 0, 0}), m_bg(w * h, {0, 0, 0}), m_fg_reset(w * h, true),
        m_bg_reset(w * h, true), m_style(w * h, style::reset) {}
  ...
  void set_fg(size_t x, size_t y, unsigned int r, unsigned int g,
              unsigned int b) {
    m_fg_reset[(y - 1) * w + (x - 1)] = false;
    m_fg[(y - 1) * w + (x - 1)] = {r, g, b};
  }

  void set_bg(size_t x, size_t y, unsigned int r, unsigned int g,
              unsigned int b) {
  ...
  void fill_fg(int x1, int y1, int x2, int y2, unsigned int r, unsigned int g,
               unsigned int b) {
    for (int j = y1; j <= y2; j++) {
      for (int i = x1; i <= x2; i++) {
        set_fg(i, j, r, g, b);
  ...
  void fill_bg(int x1, int y1, int x2, int y2, unsigned int r, unsigned int g,
               unsigned int b) {
    for (int j = y1; j <= y2; j++) {
      for (int i = x1; i <= x2; i++) {
        set_bg(i, j, r, g, b);
...
static void render(Term::Window &scr, const Model &m, size_t cols) {
  ...
  scr.print_str(cols - 6, m.lines.size(),
                std::to_string(m.cursor_row) + "," +
  ...
  while (not_complete) {
    ...
    if ((key >= 'a' && key <= 'z') || (key >= 'A' && key <= 'Z') ||
      ...
      case Key::BACKSPACE:
        if (m.cursor_col > 1) {
          std::string before =
              m.lines[m.cursor_row - 1].substr(0, m.cursor_col - 2);
          std::string after =
